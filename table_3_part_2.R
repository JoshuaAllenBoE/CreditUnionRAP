#' @title Create table 3, part 2 from the credit union publication
#'
#' @description Creates Table 3, part 2 from the \href{https://www.bankofengland.co.uk/news?NewsTypes=571948d14c6943f7b5b7748ad80bef29&Direction=Upcoming}{Credit Union Quarterly Release}.
#'
...
table_3_part_2<- function(x) {
  ...
  x <- x %>%
    dplyr::arrange(desc(Quarter)) %>%
    dplyr::filter(Data.Element %in% c("CQ_K18")) %>%
    dplyr:: select( Quarter, Country, Year.End, Position) %>%
    dplyr:: group_by( Country, Quarter, Year.End) %>%
    dplyr:: summarise(Position = sum(Position)/1000) %>%#convert into 1000s
    dplyr:: ungroup() %>%
...
#View(table_3_part_2(CQ_Combine)[[1]])
