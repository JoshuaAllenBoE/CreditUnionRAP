#' @title Create table 4, part 1 from the credit union publication
#'
#' @description Creates Table 4, part 1 from the \href{https://www.bankofengland.co.uk/news?NewsTypes=571948d14c6943f7b5b7748ad80bef29&Direction=Upcoming}{Credit Union Quarterly Release}.
#'
...
table_4_part_1<- function(x) {

  x <- x  %>%
    dplyr::arrange(desc(Quarter)) %>%

    dplyr::filter(Data.Element %in% c("CQ_H4", "CQ_H6","CQ_H8","CQ_H10")) %>%
    ...
    dplyr:: mutate(Arrears = CQ_H4 + CQ_H6 + CQ_H8 + CQ_H10)   %>%
    dplyr:: select(FRN, Country, Quarter, Arrears) %>%
    dplyr:: summarise(Arrears = sum(Arrears)/1000) %>% # convert value of arrears in 1000s
    dplyr:: select( Quarter, Country, Arrears) %>%
    dplyr:: slice(1:5) %>%
    dplyr:: mutate(Data.Element = "CQ_H4 + CQ_H6 + CQ_H8 + CQ_H10") %>%
